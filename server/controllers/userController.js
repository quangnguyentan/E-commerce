...
const fogotPassword = asyncHandler(async (req, res) => {
  ...
  await user.save();
  const html = ` 
    Xin click vào link dưới đây để thay đổi mật khẩu của bạn.Link sẽ hết hạn sau 15 phút kể từ bây giờ <a href = "${process.env.CLIENT_URL}/reset-password/${resetToken}">Click here</a> 
  `;
  const data = {
    email,
    html,
